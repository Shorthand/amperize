...
describe("Amperize", function () {
  ...
  describe("#parse", function () {
    var resetProbeImageSize, imageSizeMock, probeImageSizeStub;
    ...
    it("transforms small <img> into <amp-img></amp-img> with full image dimensions and fixed layout", function (done) {
      ...
      probeImageSizeStub.returns(
        Promise.resolve({ width: 50, height: 50, type: "jpg" })
      );
      resetProbeImageSize = Amperize.__set__(
        "probeImageSize",
        probeImageSizeStub
      );

      amperize.parse(
        '<img src="http://static.wixstatic.com/media/355241_d31358572a2542c5a44738ddcb59e7ea.jpg_256">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
            'src="http://static.wixstatic.com/media/355241_d31358572a2542c5a44738ddcb59e7ea.jpg_256"'
          );
    ...
    it("transforms big <img> into <amp-img></amp-img> with full image dimensions and responsive layout", function (done) {
      ...
      probeImageSizeStub.returns(
        Promise.resolve({ width: 350, height: 200, type: "jpg" })
      );
      resetProbeImageSize = Amperize.__set__(
        "probeImageSize",
        probeImageSizeStub
    ...
    it("transforms <img> into <amp-img></amp-img> when width and height is set and overwrites it", function (done) {
      ...
      amperize.parse(
        '<img src="http://somestockwebsite.com/image.jpg" width="100" height="50">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
            'src="http://somestockwebsite.com/image.jpg"'
          );
          expect(result).to.contain('layout="responsive"');
          expect(result).to.contain('width="350"');
          expect(result).to.contain('height="200"');
    ...
    it("transforms <img> into <amp-img></amp-img> does not overwrite layout attribute", function (done) {
      ...
      amperize.parse(
        '<img src="http://somestockwebsite.com/image.jpg" layout="fixed">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
    ...
    it("transforms <img> into <amp-img> when no file extension is given", function (done) {
      ...
      probeImageSizeStub.returns(
        Promise.resolve({ width: 104, height: 15, type: "png" })
      );
      resetProbeImageSize = Amperize.__set__(
        "probeImageSize",
        probeImageSizeStub
      ...
      amperize.parse(
        '<img src="https://www.zomato.com/logo/18163505/minilogo">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
            'src="https://www.zomato.com/logo/18163505/minilogo"'
          );
          expect(result).to.contain('layout="fixed"');
    ...
    it("falls back to image-size for unprobable images", function (done) {
      ...
      amperize.parse(
        '<img src="https://somewebsite.com/favicon.ico">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
            'src="https://somewebsite.com/favicon.ico"'
          );
          expect(result).to.contain('layout="fixed"');
    ...
    it("falls back to image-size for unprobable images (query param)", function (done) {
      ...
      amperize.parse(
        '<img src="https://somewebsite.com/favicon.ICO?v=1">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
          expect(result).to.contain(
            'src="https://somewebsite.com/favicon.ICO?v=1"'
          );
          expect(result).to.contain('layout="fixed"');
          expect(result).to.contain('width="256"');
          expect(result).to.contain('height="256"');
    ...
    it("transforms .gif <img> with only height property into <amp-anim></amp-anim> with full dimensions by overriding them", function (done) {
      ...
      probeImageSizeStub.returns(
        Promise.resolve({ width: 800, height: 600, type: "gif" })
      );
      resetProbeImageSize = Amperize.__set__(
        "probeImageSize",
        probeImageSizeStub
      ...
      amperize.parse(
        '<img src="https://media.giphy.com/media/l46CtzgjhTm29Cbjq/giphy.gif" height="500">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-anim");
          expect(result).to.contain(
            'src="https://media.giphy.com/media/l46CtzgjhTm29Cbjq/giphy.gif"'
          );
          expect(result).to.contain('layout="responsive"');
    ...
    it("transforms <iframe> with only width property into <amp-iframe></amp-iframe> with full dimensions without overriding them", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" width="400"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain(
            'src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA"'
          );
          expect(result).to.contain('layout="responsive"');
    ...
    it("transforms <iframe> with only height property into <amp-iframe></amp-iframe> with full dimensions without overriding them", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" height="400"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain(
    ...
    it("transforms <iframe> with frameborder=0 to preserve frameborder=0", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" frameborder="0"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it("transforms <iframe> with frameborder=1 to preserve frameborder=1", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" frameborder="1"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it("transforms <iframe> with frameborder to frameborder=1", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" frameborder></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain('frameborder="1"');
    ...
    it("transforms <iframe> with scrolling=0 to preserve scrolling=0", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" scrolling="0"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it("transforms <iframe> with scrolling=1 to preserve scrolling=1", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" scrolling="1"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it("transforms <iframe> with scrolling to scrolling=1", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" scrolling></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain('scrolling="1"');
    ...
    it('transforms <iframe> with allowfullscreen to allowfullscreen=""', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowfullscreen=""></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it('transforms <iframe> with allowfullscreen="true" to allowfullscreen=""', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowfullscreen="true"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain('allowfullscreen=""');
    ...
    it('transforms <iframe> with allowfullscreen="false" to remove allowfullscreen', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowfullscreen="false"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it('transforms <iframe> with allowtransparency to allowtransparency=""', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowtransparency></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it('transforms <iframe> with allowtransparency="true" to allowtransparency=""', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowtransparency="true"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain('allowtransparency=""');
    ...
    it('transforms <iframe> with allowtransparency="false" to remove allowtransparency', function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" allowtransparency="false"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
    ...
    it("transforms <iframe> with youtube URL to <amp-youtube></amp-youtube>", function (done) {
      amperize.parse(
        '<iframe src="https://www.youtube.com/embed/HMQkV5cTuoY" height="400"></iframe>',
        function (error, result) {
          expect(result).to.exist;
    ...
    it("transforms <iframe> with youtube URL to <amp-youtube></amp-youtube> removing disallowed attributes", function (done) {
      amperize.parse(
        '<iframe src="https://www.youtube.com/embed/HMQkV5cTuoY" allowfullscreen frameborder="0" allow="" height="400"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-youtube");
          expect(result).to.contain('data-videoid="HMQkV5cTuoY"');
    ...
    it("transforms <iframe> with sandbox property into <amp-iframe></amp-iframe> with full dimensions without overriding them", function (done) {
      amperize.parse(
        '<iframe src="https://giphy.com/embed/3oEduKP4VaUxJvLwuA" sandbox="allow-scripts"></iframe>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-iframe");
          expect(result).to.contain(
    ...
    it("adds 'https' protocol to <iframe> if no protocol is supplied (e. e. giphy)", function (done) {
      var url =
        '<iframe src="//giphy.com/embed/3oEduKP4VaUxJvLwuA" width="480" height="372" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>';
    ...
    it("adds 'https' protocol to <iframe> if only 'http' protocol is supplied", function (done) {
      var url =
        '<iframe src="http://giphy.com/embed/3oEduKP4VaUxJvLwuA" width="480" height="372" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="http://giphy.com/gifs/afv-funny-fail-lol-3oEduKP4VaUxJvLwuA">via GIPHY</a></p>';
      amperize.parse(url, function (error, result) {
        expect(result).to.exist;
        expect(result).to.contain("<amp-iframe");
        expect(result).to.contain(
    ...
    it("transforms local <img> into <amp-img></amp-img> with default image dimensions", function (done) {
      amperize.parse(
        '<img src="/content/images/IMG_xyz.jpg">',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-img");
    ...
    it("can handle <img> tag without src and does not transform it", function (done) {
      const expectedContent = "<img><!--/img--><p>some text here</p>";
      ...
      amperize.parse(
        ...
        function (error, result) {
          ...
          expect(result).to.exist;
          expect(result).to.be.equal(expectedContent);
          done();
        }
      );
    });
    ...
    it("transforms <audio> with a fallback to <amp-audio>", function (done) {
      amperize.parse(
        '<audio src="http://foo.mp3" autoplay>Your browser does not support the <code>audio</code> element.</audio>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain(
            '<amp-audio src="https://foo.mp3" autoplay="">'
          );
          expect(result).to.contain(
    ...
    it("transforms <audio> with a <source> tag to <amp-audio> and maintains the attributes", function (done) {
      amperize.parse(
        '<audio controls="controls" width="auto" height="50" autoplay="mobile">Your browser does not support the <code>audio</code> element.<source src="//foo.wav" type="audio/wav"></audio>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain("<amp-audio");
          expect(result).to.contain(
            'controls="controls" width="auto" height="50" autoplay="mobile"'
          );
          expect(result).to.contain(
            '<source src="https://foo.wav" type="audio/wav">'
          );
          expect(result).to.contain("</amp-audio>");
          done();
        }
    ...
    it("transforms <audio> with a <track> tag to <amp-audio>", function (done) {
      amperize.parse(
        '<audio src="foo.ogg"><track kind="captions" src="https://foo.en.vtt" srclang="en" label="English"><track kind="captions" src="https://foo.sv.vtt" srclang="sv" label="Svenska"></audio>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain('<amp-audio src="foo.ogg">');
          expect(result).to.contain(
            '<track kind="captions" src="https://foo.en.vtt" srclang="en" label="English">'
          );
          expect(result).to.contain(
            '<track kind="captions" src="https://foo.sv.vtt" srclang="sv" label="Svenska">'
          );
          expect(result).to.contain("</amp-audio>");
          done();
        }
    ...
    it("transforms all src urls in <amp-audio> to https", function (done) {
      amperize.parse(
        '<audio src="//foo.ogg"><source type="audio/mpeg" src="http://foo.mp3"><track kind="captions" src="http://foo.en.vtt" srclang="en" label="English"><track kind="captions" src="http://foo.sv.vtt" srclang="sv" label="Svenska"></audio>',
        function (error, result) {
          expect(result).to.exist;
          expect(result).to.contain('<amp-audio src="https://foo.ogg">');
          expect(result).to.contain(
            '<source type="audio/mpeg" src="https://foo.mp3">'
          );
          expect(result).to.contain(
            '<track kind="captions" src="https://foo.en.vtt" srclang="en" label="English">'
          );
    ...
    it("can handle redirects", function (done) {
      ...
      imageSizeMock = nock("http://noimagehere.com")
        ...
        .reply(
          ...
          {
            location:
              "http://someredirectedurl.com/files/f/feedough/x/11/1540353_20925115.jpg",
          }
        );
      ...
      amperize.parse(
        '<img src="http://noimagehere.com/files/f/feedough/x/11/1540353_20925115.jpg">',
        function (error, result) {
          expect(imageSizeMock.isDone()).to.be.equal(
            true,
            "imageSizeMock isn't done"
          );
          expect(secondImageSizeMock.isDone()).to.be.equal(
            true,
            "secondImageSizeMock isn't done"
          );
          expect(error).to.be.null;
          expect(result).to.contain(
            '<amp-img src="http://noimagehere.com/files/f/feedough/x/11/1540353_20925115.jpg" width="1" height="1" layout="fixed"></amp-img>'
          );
          done();
        }
      );
    ...
    it("can handle request errors", function (done) {
      ...
      amperize.parse(
        '<img src="http://example.com/images/IMG_xyz.jpg">',
        function (error, result) {
          expect(error).to.be.null;
          expect(result).to.contain(
            '<img src="http://example.com/images/IMG_xyz.jpg">'
