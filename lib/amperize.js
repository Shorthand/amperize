...
const FETCH_ONLY_FORMATS = ["cur", "icns", "ico", "dds"];
...
Amperize.prototype.traverse = async function traverse(data, html, done) {
  ...
  function _probeImageSize(url) {
    return probeImageSize(url, requestOptions).then(function (result) {
  ...
  function _fetchImageSize(url) {
    return axios
      ...
      .then(function (response) {
        var result = sizeOf(response.data);
        imageSizeCache[url] = result;
        return result;
      });
  }
    ...
    function reduce(html, element, step) {
      ...
      if (element.name === "iframe") {
        ...
        if (
          ...
          element.attribs.width = !element.attribs.width
            ? self.config["amp-iframe"].width
            : element.attribs.width;
          element.attribs.height = !element.attribs.height
            ? self.config["amp-iframe"].height
            : element.attribs.height;
          setLayoutAttribute(element);
        }
      }
